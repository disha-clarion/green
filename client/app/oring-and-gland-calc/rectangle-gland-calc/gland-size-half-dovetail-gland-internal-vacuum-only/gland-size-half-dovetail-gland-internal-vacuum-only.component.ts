...
import { ControlType, Units, ControlSubType, GlandSizeCalc_HalfDovetailGlandVacuumOnly_Validators, CalculatorTypes, calculatedDataKeys } from '../../../shared/helpers/constants';
...
export class GlandSizeHalfDovetailGlandInternalVacuumOnlyComponent implements OnInit, OnDestroy {
  calculatorConfig: CalculatorModel;
  glandSizeHalfDovetailGlandForm: FormGroup;
  ...
  constructor(
    ...
    private glandSizeHalfDovetailGlandInternalVacuumOnlyService: GlandSizeHalfDovetailGlandInternalVacuumOnlyService,
  ...
  private initializeOringConfig(glandSizeJsonConfig: BaseModel) {
    ...
    const glandSizeDovetailGlandVacuumOnly = { ...glandSizeJsonConfig.calculators[2] };
  ...
  getControlValue(fieldName: string) {
    if (this.glandSizeHalfDovetailGlandForm
      ...
      return (+this.glandSizeHalfDovetailGlandForm.controls[fieldName].value).toFixed(this.unitButtonService.getCurrentUnitValue.id === Units.MILLI_METER ? 2 : 3);
    }
  }
  ...
  createForm() {
    ...
    const leftFormControls = this.helperService.mapFormControlsOnCreateForm(this.glandSizeHalfDovetailGlandForm, this.calculatorConfig.form.left_column.form_layout_row);
    const rightFormControls = this.helperService.mapFormControlsOnCreateForm(this.glandSizeHalfDovetailGlandForm, this.calculatorConfig.form.right_column.form_layout_row);
    ...
    for (let index = 0; index < jsonConfigValidationsArray.length; index++) {
      ...
      switch (validationFuncKey) {
        case GlandSizeCalc_HalfDovetailGlandVacuumOnly_Validators.OPERATING_TEMPERATURE_VALIDATION:
          validations.push(this.rectangleGlandCalcValidationService.operatingTemperatureValidation());
          break;
        case GlandSizeCalc_HalfDovetailGlandVacuumOnly_Validators.O_RING_CROSS_NOMINAL_AS568A_STANDARD_SECTION_VALIDATION:
          validations.push(this.rectangleGlandCalcValidationService.oringCrossSectionNominalValidation());
          break;
        case GlandSizeCalc_HalfDovetailGlandVacuumOnly_Validators.ORING_ID_AS568A_STANDARD_WARNING:
          validations.push(this.rectangleGlandCalcValidationService.oRingIDAS568AStandardsWarning());
          break;
        case GlandSizeCalc_HalfDovetailGlandVacuumOnly_Validators.O_RING_ID_NOMINAL_SHOULD_BE_LESS_THAN_MAX_ORING_ID_VALUE_VALIDATION:
  ...
  onSubmit(isSubmitted = false) {
    ...
    // perform calculation
    this.glandSizeHalfDovetailGlandInternalVacuumOnlyService.createGland(
      this.glandSizeHalfDovetailGlandForm,
  ...
  onCopy() {
    ...
    if (this.CalculatedData) {
      formAndOutputData = { ...this.glandSizeHalfDovetailGlandForm.value, ...this.CalculatedData };
    }
    else {
      formAndOutputData = { ...this.glandSizeHalfDovetailGlandForm.value };
    }

    formAndOutputData.calculatorType = CalculatorTypes.ORingHalfDovetailGlandInternalVacuumOnly;
    localStorage.setItem(calculatedDataKeys.HALF_DOVETAIL_GLAND, JSON.stringify(formAndOutputData));
